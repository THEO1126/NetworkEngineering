...
```shell
port link-type access     # 端口模式设为Access
port default vlan 10      # 将端口加入到VLAN 10
```

- **行为逻辑**：
    - ❗**接收方向**： 无论收到什么帧，都强制认为它属于 VLAN 10，并打上VLAN 10的标签。
    - ❗**发送方向**： 所有要从该端口发出的、属于VLAN 10的帧，**在发出前一定会被剥离VLAN标签**。它**永远不会**发送带标签的帧。


- **结果**： 连接在这个口上的终端设备完全感知不到VLAN的存在，它只收发普通的以太网帧（无标签帧）。

**适用场景**：**连接最终用户终端设备**，这些设备不需要也不理解VLAN标签。
...
**PVID的重要性**：
- 在Trunk口上，PVID决定了无标签帧的处理方式
- 如果PVID不在允许列表，无标签帧会被丢弃

❗**未设置PVID**
```shell
port link-type trunk
port trunk allow-pass vlan 10 20
# 没有设置trunk pvid，使用默认PVID（通常是VLAN 1）
```
1. **假设PVID默认是VLAN 1**：
    - 接收无标签帧 → 打上VLAN 1标签
    - 但是VLAN 1**不在允许列表中** → **帧被丢弃！**

2. **如果PVID=1且只允许VLAN 10**：
    - 这个端口**几乎无法工作**！
    - 因为它会丢弃所有无标签帧（标记为VLAN 1后被丢弃）
    - 只能接收VLAN 10的带标签帧


❗**PVID**
>Trunk端口的PVID只能**设置一个VLAN ID**
```shell
port link-type trunk
port trunk allow-pass vlan 10 20
trunk pvid vlan 10
```
1. **接收方向**：
    - 收到**无标签帧** → 打上VLAN 10标签（因为PVID=10）
    - 收到**VLAN 10或20带标签帧** → 正常接收
    - 收到**其他VLAN带标签帧** → 丢弃（只允许VLAN 10 20）
    
2. **发送方向**：
    - 发送**VLAN 10的帧** → 剥离标签，以无标签帧发送（因为是PVID）
    - 发送**其他VLAN的帧** → 带标签发送（VLAN 20）
...
**allow-pass**命令的作用：
- **仅允许VLAN通过，不指定标签行为**
- 但Hybrid端口**必须**为每个VLAN指定`untagged`或`tagged`
- allow-pass 在Hybrid端口上是多余的

在华为/华三设备上，Hybrid端口的**每个VLAN必须明确指定标签行为**：
- `port hybrid untagged vlan X` → 允许VLAN X通过，且无标签发送
- `port hybrid tagged vlan X` → 允许VLAN X通过，且带标签发送
**关键点**：`untagged`和`tagged`命令本身就**隐含了"允许通过"**！

❗ Trunk端口和Hybrid端口的一个**核心区别**：**Trunk端口的PVID只能设置一个VLAN ID**

需要多个VLAN都以无标签方式发送时：
```shell
# 允许VLAN 10和20，都设置为无标签发送
port link-type hybrid
port hybrid pvid vlan 10        # 关键1：接收无标签帧标记为VLAN 10
port hybrid untagged vlan 10    # 关键2：发送VLAN 10帧时剥离标签（与PVID一致）
port hybrid tagged vlan 20      # 关键3：发送VLAN 20帧时保留标签
# 虽然Hybrid端口的PVID也只能设一个，但可以通过`untagged`命令让多个VLAN都以无标签方式发送
❗ #设置了pvid的VLAN，必须设置untagged或者tagged，否则该VLAN无法从该端口发送出去，因为不在端口的允许列表中
```
...
**1、PVID的定义**
PVID（Port VLAN ID）即**端口默认VLAN ID**，它的作用是：
- 当端口收到**无标签帧**时，自动将其标记为该PVID
- 当发送属于PVID的帧时，会**剥离标签**发送

**2、Trunk端口的固定规则**
Trunk端口有一个固定的行为模式：
- **PVID对应的VLAN** → 发送时**无标签**（untagged）
- **其他所有VLAN** → 发送时**带标签**（tagged）
这个规则决定了Trunk端口**只能有一个VLAN是无标签的**，因此PVID只能是单个VLAN。


**3、Trunk与Hybrid在PVID方面的对比**

| 特性            | Trunk端口                                | Hybrid端口                 |
| ------------- | -------------------------------------- | ------------------------ |
| **PVID数量**    | **只能1个**                               | **只能1个**                 |
| **无标签VLAN数量** | **只能1个**（就是PVID）                       | **可以多个**（通过`untagged`配置） |
| **PVID作用**    | 1. 接收无标签帧→标记为PVID  <br>2. 发送PVID帧→剥离标签 | 同Trunk                   |
| **标签控制**      | 固定：PVID无标签，其他带标签                       | 灵活：可单独指定每个VLAN的标签行为      |
#### untagged 和 tagged

**只控制发送方向（出口）**，不控制接收方向！

- **有标签（Tagged）**：发送时，带上VLAN号
- **无标签（Untagged）**：发送时，剥离VLAN号

| 特性         | `untagged`             | `tagged`                         |
| ---------- | ---------------------- | -------------------------------- |
| **工作方向**   | **发送方向**               | **发送方向**                         |
| **发送时的标签** | **剥离标签**（无标签）          | **保留标签**（带标签）                    |
| **接收时的标签** | 不控制（允许带/无标签帧进入）        | 不控制（允许带/无标签帧进入）                  |
| **适用对端设备** | 不支持VLAN的设备（PC、打印机）     | 支持VLAN的设备（交换机、路由器）               |
| **数据帧示例**  | `[目标MAC][源MAC][数据]...` | `[目标MAC][源MAC][802.1Q标签][数据]...` |
| **标签位置**   | 无标签字段                  | 在源MAC后插入4字节标签                    |
| **目的**     | 让终端设备无需处理VLAN          | 让网络设备识别VLAN归属                    |
## 直连和旁挂区别

**一、定义：**

**旁挂：**
- 旁挂是指将**设备（如无线控制器AC）连接到网络中的某个节点旁边**，而不是直接串联在主通信路径上。在旁挂模式下，AP（接入点）的业务数据可以不经过AC而直接到达上层网络，AC主要负责对AP的管理功能，如配置管理和用户接入认证。管理流通过CAPWAP隧道传输，这样可以减轻AC的负担。 
![[Pasted image 20251223220759.png]]
**直连：**
- 直连是指**AP、AC与上层网络串联在一起，所有数据必须通过AC到达上层网络**。在这种模式下，AC同时承担数据转发和管理的功能，所有的业务流量都经过AC进行处理和转发，这样增加了AC的工作负担，对其性能要求较高。
...
### 直接转发
- 直接转发方式是指用户的数据报文到达AP后，**不经过CAPWAP**的隧道封装而直接转发到上层网络。
...
| 生成树协  议 | 特点                                                                                                      | 应用场景                                                                   |
| ------- | ------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| STP     | ● 形成一棵无环路的树，解决广  <br>播风暴并实现冗余备份。  <br>● 收敛速度较慢。                                                         | 无需区分用户或业务流量，所有  <br>VLAN共享一棵生成树。                                       |
| RSTP    | ● 形成一棵无环路的树，解决广  <br>播风暴并实现冗余备份。  <br>● 收敛速度快。                                                          | 无需区分用户或业务流量，所有  <br>VLAN共享一棵生成树。                                       |
| MSTP    | ● 形成多棵无环路的树，解决广  <br>播风暴并实现冗余备份。  <br>● 收敛速度快。  <br>● 多棵生成树在VLAN间实现负载  <br>均衡，不同VLAN的流量按照不  <br>同的路径转发。 | 需要区分用户或业务流量，并实现  <br>负载分担。不同的VLAN通过不同  <br>的生成树转发流量，每棵生成树之  <br>间相互独立。 |
